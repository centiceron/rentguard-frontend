...
import { ShieldCheck, FileSignature, CheckCircle, AlertTriangle } from "lucide-react";
...
export default function SignatureRoom() {
  ...
  useEffect(() => {
    ...
    const fetchFullLease = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/lease/${id}`, {
  ...
  const handleAccept = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("http://localhost:5000/api/lease/handshake", {
        method: "POST",
        headers: {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50"><ShieldCheck className="text-indigo-500 animate-spin h-8 w-8" /></div>;
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center py-12 px-4">
      <div className="mb-8 text-center text-white">
        ...
        <p className="text-slate-400 mt-2">Review the document carefully before applying your cryptographic signature.</p>
      </div>
      ...
      <Card className="w-full max-w-4xl border-slate-700 bg-slate-800 text-white shadow-2xl relative overflow-hidden">
        {success && (
          <div className="absolute inset-0 bg-slate-900/95 backdrop-blur-sm z-50 flex flex-col items-center justify-center animate-in fade-in">
            ...
            <h2 className="text-2xl font-bold">Cryptographic Handshake Complete</h2>
          </div>
        )}
        ...
        <CardContent className="pt-6">
          <div className="bg-slate-300 p-2 rounded-lg mb-6 max-h-[60vh] overflow-y-auto">
            {leaseDetails?.pdfBase64 && (
              <iframe src={`data:application/pdf;base64,${leaseDetails.pdfBase64}`} width="100%" height="600px" className="rounded-md bg-white" />
            )}
          </div>
          ...
          {!user?.isAadhaarVerified ? (
            <div className="bg-red-900/50 border border-red-500 p-4 rounded-lg flex items-start gap-4">
              ...
              <div>
                <h3 className="font-bold text-red-200">KYC Verification Required</h3>
                <p className="text-sm text-red-300 mt-1">Your account is missing Aadhaar Verification. You cannot sign legal documents until your identity is verified.</p>
              </div>
            </div>
          ) : (
            <Button 
            ...
            >
              ...
              {isAccepting ? "Executing Ledger Entry..." : leaseDetails?.tenantId ? "Document Already Signed" : "I Agree & Sign Document"}
            </Button>
          )}
        </CardContent>
      </Card>
